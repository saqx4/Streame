...
<section class="space-y-6">
  <div class="flex flex-wrap items-end justify-between gap-3">
    <div>
      ...
      <p class="mt-1 text-sm text-white/60">Popular movies from TMDB</p>
    </div>
  ...
  {:else}
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6">
      {#each items as m (m.id)}
        <a use:link href={`/movie/${m.id}`} class="group overflow-hidden rounded-xl border border-white/10 bg-white/5">
          ...
          <div class="p-3">
            <div class="line-clamp-1 text-sm font-medium">{m.title}</div>
            <div class="mt-1 text-xs text-white/60">
              {m.release_date ? m.release_date.slice(0, 4) : '—'} • ⭐ {m.vote_average?.toFixed?.(1) ?? m.vote_average}
            </div>
          </div>
        </a>
      {/each}
